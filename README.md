...
让 Astrbot agent 获得**主动读图能力**，而非在聊天时由框架被动地注入。插件提供主动阅读工具，使得Agent可主动阅读在文件系统中发现的图片。

插件利用本地数据库设置缓存机制，当阅读相同图片（甚至被压缩过的同图片）时，将直接命中缓存并跳过对VL模型的请求，从本地数据库中阅读结果。可降低token花费，提高效率。
