...
- **降级链被「空内容」误判为成功而中断**：当首选模型调用成功但返回空 content（如 DeepSeek 官方 API 视觉接口未开放、接受请求但返回空）时，旧逻辑误判为成功并 `break` 退出整个循环，导致后续降级模型根本没有机会执行，最终误报「所有 VL 模型均未配置 api_key」。现在空内容视为该模型失败，继续降级下一个模型；错误文案同步修正为「所有 VL 模型均返回空内容」。
